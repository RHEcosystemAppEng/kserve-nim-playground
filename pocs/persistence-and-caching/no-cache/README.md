...
(cd pocs/persistence-and-caching/no-cache && kustomize edit set namespace knim-no-cache)
```
...
namespace/knim-no-cache created
...
$ k wait inferenceservices -n knim-no-cache llama3-8b-instruct-1xgpu --for condition=Ready --timeout 200s
...
k logs -n knim-no-cache $pod kserve-container | grep 'Model workspace is now ready'
...
$ runtimeurl=$(k get inferenceservices -n knim-no-cache llama3-8b-instruct-1xgpu -o yaml | yq '.status.url') && \
...
$ k get kpa -n knim-no-cache llama3-8b-instruct-1xgpu-predictor-00001 -o yaml | yq '.status' | yq '. |= pick(["actualScale", "desiredScale"])'
...
namespace "knim-no-cache" deleted
